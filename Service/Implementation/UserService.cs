...
namespace Villager.Api.Service.Implementation
{
        ...
        SignInManager<ApplicationUser> signInManager) : IUserService
    {
        public async Task<ErrorOr<UserResponseDto>> CreateUser(CreateUserDto createUserDto)
        {
            ...
            applicationUser.DateOfBirth = date.ToString();
        ...
        public async Task<ErrorOr<Deleted>> DeleteUser( string id)
        {
            ...
            if (!x.Succeeded)
            {
                return Error.Failure(description: "Unable to delete user");
            }
        ...
        public async Task<ErrorOr<UserResponseDto>> EditUser(string id, EditUserDto editUserDto)
        {
            ...
            if (!result.Succeeded)
            {
                return Error.Failure(description: "Unable to update user");
            }
        ...
        public async Task<ErrorOr<bool>> VerifyEmail(string email,string code)
        {
            var user = await FindUserByEmail(email);
            if (user.IsError)
            {
        ...
        private string GenerateToken(ApplicationUser applicationUser)
        {
            ...
            var claims = new[] {
                ...
                new Claim(System.IdentityModel.Tokens.Jwt.JwtRegisteredClaimNames.Birthdate, applicationUser.DateOfBirth.ToString()),
